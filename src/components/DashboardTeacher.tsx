...
export default function DashboardTeacher({
      ...
      try {
        ...
        if (active) {
          ...
          snap.forEach((docSnap) => {
            ...
            logs.push({ id: docSnap.id, ...data, createdAt: data.createdAt?.toDate() || null });
          });
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (filesList && filesList.length > 0) validateAndAddFiles(Array.from(filesList));
  };
  ...
  const validateAndAddFiles = (filesList: File[]) => {
    ...
    if (newValidFiles.length > 0) {
      ...
      setFileStatuses(prev => {
        const next = { ...prev };
        newValidFiles.forEach(f => { next[f.name] = 'pending'; });
        return next;
      });
    }
  ...
  const handleRemoveFile = (index: number) => {
    ...
    if (fileToRemove) {
      setFileProgresses(prev => { const next = { ...prev }; delete next[fileToRemove.name]; return next; });
      setFileStatuses(prev => { const next = { ...prev }; delete next[fileToRemove.name]; return next; });
      setFileValidationErrors(prev => { const next = { ...prev }; delete next[fileToRemove.name]; return next; });
    }
  };
  ...
  const handleUploadSubmit = async (e: React.FormEvent) => {
    ...
    if (isNewSubjectForm) {
      ...
      try { await addSubject(finalSub); } catch (addErr: any) { console.warn("Subject already exists:", addErr); }
    }
    ...
    if (!finalSub) { setUploadError(t("Subject is required. Select or type a custom subject.")); setLoading(false); return; }
    if (!uploadClassLevel) { setUploadError(t("Class level is required. Select a target class.")); setLoading(false); return; }
    if (!finalCh) { setUploadError(t("Chapter is required. Select or type a custom chapter.")); setLoading(false); return; }
    if (!finalTopic) { setUploadError(t("Topic is required. Select or type a custom topic.")); setLoading(false); return; }
    if (!finalItemType) { setUploadError(t("Item type is required. Choose preset or create custom.")); setLoading(false); return; }
    ...
    for (const file of selectedFiles) {
      ...
      try {
          ...
          if (r2Response.ok) {
            ...
            await new Promise<void>((resolve, reject) => {
              uploadTask.on('state_changed',
                (snapshot) => { setFileProgresses(prev => ({ ...prev, [file.name]: Math.round((snapshot.bytesTransferred / snapshot.totalBytes) * 100) })); },
                (error) => reject(error),
                () => resolve()
              );
            });
        ...
        } catch (logErr) { console.warn("Failed to write activity upload log:", logErr); }
  ...
  const handleSaveEdit = async () => {
    if (!editingFile || !onFileEdit) return;
    setEditSaving(true);
    try {
      ...
      {rejectionLogs.length > 0 && (
        <div className="bg-amber-50 dark:bg-amber-950/20 border-l-4 border-amber-500 p-4 rounded-r-xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 shadow-sm animate-in slide-in-from-top duration-250">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-bold text-amber-805 dark:text-amber-200">{t("Material Rejection Logs Available")}</p>
        ...
        <div className="lg:col-span-1 bg-white dark:bg-slate-900 p-6 rounded-xl border border-gray-100 dark:border-slate-800 shadow-xs self-start transition-colors">
          ...
          {loading && (
            <div className="mb-4 p-3.5 bg-blue-50 dark:bg-blue-950/25 text-blue-700 dark:text-blue-400 rounded-lg text-xs font-semibold border border-blue-105/30 space-y-2">
              <div className="flex items-center gap-2.5 animate-pulse">
                ...
                <span>{uploadProgress !== null ? `${t("Uploading...")} ${uploadProgress}%` : t("Securely transferring file archive to Cloudflare R2 pipeline...")}</span>
              </div>
              {uploadProgress !== null && (
                <div className="w-full bg-blue-100 dark:bg-slate-800 rounded-full h-2 overflow-hidden">
                  <div className="bg-brand-500 h-full transition-all duration-300 ease-out" style={{ width: `${uploadProgress}%` }} id="upload-progress-bar" />
                </div>
              )}
            </div>
          )}
          ...
          <form onSubmit={handleUploadSubmit} className="space-y-4">
            ...
            {selectedFiles.length > 0 && (
              <div className="space-y-2 mt-4 max-h-[250px] overflow-y-auto pr-1 border border-gray-100 dark:border-slate-800 rounded-xl p-3 bg-gray-50/30 dark:bg-slate-900/40">
                <div className="flex items-center justify-between pb-2 border-b border-gray-100 dark:border-slate-800">
                  <span className="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{t("Upload Queue")} ({selectedFiles.length})</span>
                  {!loading && (
                    <button type="button" onClick={() => { setSelectedFiles([]); setFileValidationErrors({}); setFileProgresses({}); setFileStatuses({}); }} className="text-[10px] font-bold text-red-500 hover:underline cursor-pointer">
                      {t("Clear All")}
                    </button>
                  )}
                </div>
                      ...
                      <div className="flex items-center justify-between gap-2">
                        <div className="flex items-center gap-2.5 min-w-0">
                          ...
                          <div className="min-w-0">
                            <p className="text-xxs font-bold text-gray-700 dark:text-gray-200 truncate max-w-[150px] sm:max-w-[200px]" title={file.name}>{file.name}</p>
                            <p className="text-[9px] text-gray-400 dark:text-gray-500 font-medium">{(file.size / (1024 * 1024)).toFixed(2)} MB</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-2 shrink-0">
                          {status === 'uploading' && (<span className="text-[10px] text-blue-600 dark:text-blue-400 font-bold flex items-center gap-1"><Loader2 className="w-3 h-3 animate-spin" />{progress}%</span>)}
                          {status === 'success' && (<span className="text-[10px] text-emerald-600 dark:text-emerald-400 font-bold flex items-center gap-1 bg-emerald-50 dark:bg-emerald-955/30 px-2 py-0.5 rounded-md"><CheckCircle2 className="w-3 h-3" />{t("Success")}</span>)}
                          {status === 'error' && (<span className="text-[10px] text-red-600 dark:text-red-400 font-bold flex items-center gap-1 bg-red-50 dark:bg-red-950/30 px-2 py-0.5 rounded-md"><AlertCircle className="w-3 h-3" />{t("Failed")}</span>)}
                          {status === 'pending' && !loading && (<button type="button" onClick={() => handleRemoveFile(idx)} className="p-1 text-gray-400 hover:text-red-500 dark:hover:text-red-400 cursor-pointer"><X className="w-3.5 h-3.5" /></button>)}
                        </div>
                      </div>
                      {status === 'uploading' && (<div className="w-full bg-gray-100 dark:bg-slate-800 h-1 rounded-full overflow-hidden mt-2"><div className="bg-brand-500 h-full transition-all duration-300" style={{ width: `${progress}%` }} /></div>)}
                      {fileErr && (<p className="text-[9px] text-red-600 dark:text-red-400 mt-1.5 font-medium leading-relaxed">⚠️ {fileErr}</p>)}
                    </div>
                  );
                })}
              </div>
            ...
            <div className="space-y-3.5 border-t border-b border-gray-105/70 dark:border-slate-800 py-3.5">
              <div>
                <div className="flex justify-between items-center mb-1.5">
                  <span className="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider flex items-center gap-1"><BookOpen className="w-3.5 h-3.5 text-brand-500" />{t("Subject")}</span>
                  <span className="text-[10px] font-bold text-amber-500 bg-amber-500/10 px-1.5 py-0.5 rounded flex items-center gap-1">🔐 {t("Authorized Only")}</span>
                </div>
                <div className="relative">
                  <select value={selectedSubject} onChange={(e) => setSelectedSubject(e.target.value)} className="w-full pl-3 pr-10 py-2.5 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 text-gray-800 dark:text-gray-100 rounded-lg focus:outline-none focus:border-brand-500 text-xs font-bold appearance-none cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed" required disabled={loading || teacherSubjects.length <= 1}>
                    <option value="">{t("-- Select Subject --")}</option>
                    {(teacherSubjects.length > 0 ? teacherSubjects : subjects).map((sub, idx) => (<option key={idx} value={sub}>{t(sub)}</option>))}
                  </select>
              ...
              <div>
                <div className="flex justify-between items-center mb-1.5">
                  <span className="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider flex items-center gap-1"><Layers className="w-3.5 h-3.5 text-brand-500" />{t("Class")}</span>
                </div>
                <div className="relative">
                  {(() => {
                    ...
                    return (
                      <>
                        <select value={uploadClassLevel} onChange={(e) => setUploadClassLevel(e.target.value)} className="w-full pl-3 pr-10 py-2.5 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 text-gray-800 dark:text-gray-100 rounded-lg focus:outline-none focus:border-brand-500 text-xs font-bold appearance-none cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed" required disabled={loading || assignedClasses.length <= 1}>
                          <option value="">{t("-- Select Class --")}</option>
                          {assignedClasses.map((cls, idx) => (<option key={idx} value={cls}>{t(cls)}</option>))}
                        </select>
                        <span className="absolute inset-y-0 right-0 pr-3.5 flex items-center text-gray-400 pointer-events-none"><ChevronDown className="w-3.5 h-3.5" /></span>
                      </>
                    );
              ...
              <div>
                <div className="flex justify-between items-center mb-1.5">
                  <span className="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider flex items-center gap-1"><Layers className="w-3.5 h-3.5 text-brand-500" />{t("Chapter")}</span>
                  <button type="button" disabled={loading} onClick={() => { setIsNewChapterForm(!isNewChapterForm); setChapter(''); setNewChapterText(''); }} className={`text-[10px] font-bold text-brand-500 hover:underline cursor-pointer ${loading ? 'opacity-40 cursor-not-allowed' : ''}`}>
                ...
                {isNewChapterForm ? (
                  ...
                  <div className="relative">
                    <select value={chapter} onChange={(e) => setChapter(e.target.value)} className="w-full pl-3 pr-10 py-2.5 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 text-gray-800 dark:text-gray-100 rounded-lg focus:outline-none focus:border-brand-500 text-xs font-bold appearance-none cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed" required disabled={loading}>
                      ...
                      {existingChapters.map((ch, idx) => (<option key={idx} value={ch}>{ch}</option>))}
              ...
              <div>
                <div className="flex justify-between items-center mb-1.5">
                  <span className="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider flex items-center gap-1"><Sparkles className="w-3.5 h-3.5 text-brand-500" />{t("Topic")}</span>
                  <button type="button" disabled={loading} onClick={() => { setIsNewTopicForm(!isNewTopicForm); setTopic(''); setNewTopicText(''); }} className={`text-[10px] font-bold text-brand-500 hover:underline cursor-pointer ${loading ? 'opacity-40 cursor-not-allowed' : ''}`}>
                ...
                {isNewTopicForm ? (
                  ...
                  <div className="relative">
                    <select value={topic} onChange={(e) => setTopic(e.target.value)} className="w-full pl-3 pr-10 py-2.5 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 text-gray-800 dark:text-gray-100 rounded-lg focus:outline-none focus:border-brand-500 text-xs font-bold appearance-none cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed" required disabled={loading}>
                      ...
                      {existingTopics.map((tp, idx) => (<option key={idx} value={tp}>{tp}</option>))}
              ...
              <div>
                <div className="flex justify-between items-center mb-1.5">
                  <span className="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider flex items-center gap-1"><FolderLock className="w-3.5 h-3.5 text-brand-500" />{t("Note/Resource Type")}</span>
                  <button type="button" disabled={loading} onClick={() => { setIsNewItemTypeForm(!isNewItemTypeForm); setItemType(''); setNewItemTypeText(''); }} className={`text-[10px] font-bold text-brand-500 hover:underline cursor-pointer ${loading ? 'opacity-40 cursor-not-allowed' : ''}`}>
                    {isNewItemTypeForm ? t("Choose Preset") : t("+ Custom Item Type")}
                  </button>
                </div>
                {isNewItemTypeForm ? (
                  <input type="text" value={newItemTypeText} onChange={(e) => setNewItemTypeText(e.target.value)} placeholder={t("e.g. Word Meaning, Short Question...")} className="w-full px-4 py-2 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 text-gray-800 dark:text-gray-100 rounded-lg focus:outline-none focus:border-brand-500 text-xs font-semibold disabled:opacity-60 disabled:cursor-not-allowed" required disabled={loading} />
                ) : (
                  <div className="relative">
                    <select value={itemType} onChange={(e) => setItemType(e.target.value)} className="w-full pl-3 pr-10 py-2.5 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 text-gray-800 dark:text-gray-100 rounded-lg focus:outline-none focus:border-brand-500 text-xs font-bold appearance-none cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed" required disabled={loading}>
                      <option value="">{t("-- Select Note ItemType --")}</option>
                      {PRESET_ITEM_TYPES.map((it, idx) => (<option key={idx} value={it}>{t(it)}</option>))}
                    </select>
                    <span className="absolute inset-y-0 right-0 pr-3.5 flex items-center text-gray-400 pointer-events-none"><ChevronDown className="w-3.5 h-3.5" /></span>
                  </div>
                )}
            ...
            <div>
              ...
              <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder={t("Briefly state target topics, chapters, and summary of the note...")} rows={3} className="w-full px-4 py-2 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 text-gray-805 dark:text-gray-100 rounded-lg focus:outline-none focus:border-brand-500 text-xs font-semibold disabled:opacity-60 disabled:cursor-not-allowed" disabled={loading} />
            </div>

            <button type="submit" disabled={loading || selectedFiles.length === 0} className={`w-full py-2.5 rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-1.5 cursor-pointer ${(selectedFiles.length > 0 && !loading) ? 'bg-brand-500 text-white hover:bg-brand-600 shadow-md' : 'bg-gray-100 dark:bg-slate-800 text-gray-400 dark:text-gray-600 cursor-not-allowed opacity-50'}`}>
              {loading ? (<><Loader2 className="w-3.5 h-3.5 animate-spin" /><span>{t("Uploading...")}</span></>) : (<><Sparkles className="w-3.5 h-3.5" /><span>{t("Upload Resource File")}</span></>)}
            </button>
          </form>
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white dark:bg-slate-900 rounded-xl p-5 border border-gray-100 dark:border-slate-800 shadow-xs space-y-4 transition-colors">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 pb-3 border-b border-gray-50 dark:border-slate-800/55">
              <div className="flex flex-wrap bg-gray-100 dark:bg-slate-800/60 p-1 rounded-xl w-fit border border-gray-150/40 dark:border-slate-800 gap-1">
                <button type="button" onClick={() => { setArchiveTab('my_submissions'); setSelectedFileIds([]); }} className={`px-4 py-2 rounded-lg text-xs font-bold transition-all cursor-pointer ${archiveTab === 'my_submissions' ? 'bg-white dark:bg-slate-700 text-gray-800 dark:text-gray-100 shadow-xs' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'}`}>
                  {t("My Submissions")} ({myUploadedFiles.length})
                </button>
                <button type="button" onClick={() => { setArchiveTab('pending'); setSelectedFileIds([]); }} className={`px-4 py-2 rounded-lg text-xs font-bold transition-all cursor-pointer ${archiveTab === 'pending' ? 'bg-amber-500 text-white shadow-xs' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'}`}>
                ...
                <button type="button" onClick={() => { setArchiveTab('recent_activity'); setSelectedFileIds([]); }} className={`px-4 py-2 rounded-lg text-xs font-bold transition-all cursor-pointer ${archiveTab === 'recent_activity' ? 'bg-white dark:bg-slate-700 text-gray-800 dark:text-gray-100 shadow-xs' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'}`}>
                  {t("Recent Activity")}
                </button>
              </div>
              ...
              <div className="flex items-center gap-1.5 bg-gray-100 dark:bg-slate-800/60 p-1 rounded-xl w-fit border border-gray-150/40 dark:border-slate-800 self-end sm:self-auto">
                <button type="button" onClick={() => setViewMode('grid')} className={`p-2 rounded-lg transition-all cursor-pointer ${viewMode === 'grid' ? 'bg-white dark:bg-slate-700 text-[#15803d] dark:text-brand-400 shadow-xs' : 'text-gray-400 dark:text-gray-500 hover:text-gray-650 dark:hover:text-gray-300'}`} title={t("Grid View")}><Grid className="w-4 h-4" /></button>
                <button type="button" onClick={() => setViewMode('list')} className={`p-2 rounded-lg transition-all cursor-pointer ${viewMode === 'list' ? 'bg-white dark:bg-slate-700 text-[#15803d] dark:text-brand-400 shadow-xs' : 'text-gray-400 dark:text-gray-500 hover:text-gray-650 dark:hover:text-gray-300'}`} title={t("List View")}><List className="w-4 h-4" /></button>
              </div>
            </div>
            ...
            <div className="space-y-3">
              <div className="flex flex-col md:flex-row items-center gap-3">
                <div className="relative w-full md:flex-1">
                  <span className="absolute inset-y-0 left-0 pl-3.5 flex items-center text-gray-400 pointer-events-none"><Search className="w-4 h-4 text-gray-450 dark:text-slate-500" /></span>
                  <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder={archiveTab === 'my_submissions' ? t("Search my submissions by name, topic...") : archiveTab === 'recent_activity' ? t("Search recently active files...") : t("Search department files by name, author...")} className="w-full pl-10 pr-4 py-2.5 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-750 rounded-lg focus:outline-none focus:border-[#15803d] dark:focus:border-brand-500 text-xs font-semibold text-gray-700 dark:text-gray-200" />
                  {searchTerm && (<button onClick={() => setSearchTerm('')} className="absolute inset-y-0 right-0 pr-3 flex items-center text-gray-450 hover:text-gray-600 dark:hover:text-gray-200 text-xs font-bold cursor-pointer">{t("Clear")}</button>)}
                </div>

                <div className="relative w-full md:w-48">
                  <select value={filterSubject} onChange={(e) => setFilterSubject(e.target.value)} className="w-full pl-3 pr-10 py-2.5 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-750 text-gray-700 dark:text-gray-200 rounded-lg focus:outline-none focus:border-[#15803d] dark:focus:border-brand-500 text-xs font-bold appearance-none cursor-pointer">
              ...
              <div className="flex flex-col sm:flex-row items-center gap-4 justify-between">
                <div className="flex gap-2">
                  {(filterSubject || filterClassLevel || searchTerm) && (
                    <button onClick={() => { setFilterSubject(''); setFilterClassLevel(''); setSearchTerm(''); }} className="text-xs font-bold text-red-500 hover:underline cursor-pointer">{t("Reset Filters")}</button>
                  )}
                </div>
                <div className="text-[10px] bg-brand-100 dark:bg-brand-950/40 text-[#15803d] dark:text-brand-400 px-3 py-1 rounded-full font-bold select-none capitalize border border-[#15803d]/10">
                  {archiveTab === 'my_submissions' ? `${t("My Submissions")}: ${user.subject || t("General")}` : archiveTab === 'recent_activity' ? t("Recent Department Activity Feed") : `${t("Assigned")}: ${teacherSubjects.join(', ')}`}
                </div>
              </div>
            </div>
          </div>
              ...
              <div className="bg-white dark:bg-slate-900 rounded-xl p-5 border border-gray-150/65 dark:border-slate-800 shadow-xs transition-colors space-y-4 text-left">
                <div className="flex items-center justify-between border-b border-gray-100 dark:border-slate-800 pb-2.5">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="font-extrabold text-xs uppercase tracking-wider text-gray-800 dark:text-gray-100">{t("Live Activity Feed")}</h3>
                  </div>
                  <span className="text-[10px] text-gray-400 font-mono">{recentLogs.length} {t("Recent Events")}</span>
                </div>

                {loadingRecentLogs ? (
                  <div className="text-center py-8 text-xs font-semibold text-gray-400 dark:text-gray-500 flex flex-col items-center gap-2"><Loader2 className="w-5 h-5 animate-spin text-brand-500" /><span>{t("Syncing activity logs...")}</span></div>
                ) : recentLogs.length === 0 ? (
                  <div className="text-center py-8 text-xs text-gray-400 dark:text-gray-500">{t("No recent activity found for your assigned department subjects.")}</div>
                ) : (
                  <div className="divide-y divide-gray-100/60 dark:divide-slate-850/50 max-h-[280px] overflow-y-auto pr-1">
                    {recentLogs.map((log) => {
                      ...
                      let actionText = "";
                      if (log.action === 'file_uploaded') { logIcon = <Upload className="w-4 h-4 text-blue-500" />; actionText = t("uploaded educational material"); }
                      else if (log.action === 'file_approved') { logIcon = <CheckCircle2 className="w-4 h-4 text-green-500" />; actionText = t("approved study file"); }
                      else if (log.action === 'file_rejected') { logIcon = <AlertTriangle className="w-4 h-4 text-amber-500" />; actionText = t("rejected submission"); }
                      else if (log.action === 'file_deleted') { logIcon = <Trash2 className="w-4 h-4 text-red-500" />; actionText = t("deleted study material"); }
                      return (
                        <div key={log.id} className="py-2.5 flex items-start justify-between gap-3 text-xs">
                          <div className="flex items-start gap-2.5 min-w-0">
                            <div className="p-1.5 rounded-lg bg-gray-50 dark:bg-slate-800 border border-gray-100 dark:border-slate-750 shrink-0 mt-0.5">{logIcon}</div>
                            <div className="min-w-0">
                              <p className="text-gray-800 dark:text-gray-200 leading-tight"><strong className="font-bold">{log.actorName}</strong> <span className="text-gray-500">{actionText}</span></p>
                              <p className="font-bold text-[11px] text-brand-600 dark:text-brand-405 truncate max-w-[280px] sm:max-w-[450px] mt-0.5" title={log.fileName}>{log.fileName}</p>
                          ...
                          {matchingFile && (
                            <div className="flex items-center gap-1.5 shrink-0">
                              <button onClick={() => onDownload(matchingFile)} className="p-1.5 bg-green-50 hover:bg-green-100 dark:bg-green-950/20 text-green-600 dark:text-green-400 rounded-lg transition-colors cursor-pointer" title={t("Download File")}><Download className="w-3.5 h-3.5" /></button>
                            </div>
                          )}
                        </div>
                      );
                      ...
                      <thead>
                        <tr className="bg-gray-50 dark:bg-slate-850/30 text-gray-400 uppercase font-extrabold text-[9px] tracking-wider border-b border-gray-100 dark:border-slate-800">
                          <th className="py-3 px-4 w-12 text-center">
                            <input type="checkbox" checked={selectedFileIds.length > 0 && selectedFileIds.length === currentFilteredFiles.length} onChange={(e) => { if (e.target.checked) setSelectedFileIds(currentFilteredFiles.map(f => f.id)); else setSelectedFileIds([]); }} className="w-4 h-4 rounded border-gray-300 dark:border-slate-700 text-[#15803d] dark:text-[#22c55e] focus:ring-[#15803d] cursor-pointer accent-[#15803d]" />
                          </th>
                          ...
                          return (
                            <tr key={file.id} className="hover:bg-gray-50/50 dark:hover:bg-slate-850/15 transition-colors">
                              <td className="py-3 px-4 text-center">
                                <input type="checkbox" checked={selectedFileIds.includes(file.id)} onChange={() => { setSelectedFileIds(prev => prev.includes(file.id) ? prev.filter(id => id !== file.id) : [...prev, file.id]); }} className="w-4 h-4 rounded border-gray-300 dark:border-slate-700 text-[#15803d] dark:text-[#22c55e] focus:ring-[#15803d] cursor-pointer accent-[#15803d]" />
                              </td>
                              <td className="py-3 px-4">
                                <div className="flex items-center gap-3">
                                  <div className="p-1.5 rounded bg-gray-50 dark:bg-slate-800 border border-gray-100 dark:border-slate-700">
                                    {(file.fileType || '').toLowerCase() === 'pdf' ? (<FileText className="w-4 h-4 text-red-500" />) : ['doc', 'docx'].includes((file.fileType || '').toLowerCase()) ? (<FileText className="w-4 h-4 text-blue-500" />) : ['ppt', 'pptx'].includes((file.fileType || '').toLowerCase()) ? (<FileText className="w-4 h-4 text-orange-500" />) : ['png', 'jpg', 'jpeg'].includes((file.fileType || '').toLowerCase()) ? (<FileImage className="w-4 h-4 text-green-500" />) : (<FileText className="w-4 h-4 text-gray-500" />)}
                                  </div>
                                  <div className="max-w-[200px] sm:max-w-[250px] truncate">
                                    <p className="font-bold text-gray-800 dark:text-gray-150 truncate hover:text-[#15803d] dark:hover:text-brand-400 cursor-pointer" title={file.fileName} onClick={() => { const isPreviewable = ['pdf', 'png', 'jpg', 'jpeg', 'webp'].includes((file.fileType || '').toLowerCase()); if (isPreviewable && onPreview) onPreview(file); else onDownload(file); }}>
                                      {file.fileName}
                                    </p>
                                    {file.description && (<p className="text-[10px] text-gray-400 dark:text-gray-500 truncate" title={file.description}>{file.description}</p>)}
                                  </div>
                                </div>
                              </td>
                              <td className="py-3 px-4"><span className="text-[10.5px] font-semibold text-gray-650 dark:text-gray-350 bg-gray-100 dark:bg-slate-800 px-2 py-0.5 rounded-sm">{t(file.subject)}</span></td>
                              <td className="py-3 px-4">
                                <div className="space-y-0.5">
                                  {file.chapter && (<p className="text-[10px] text-gray-500 dark:text-gray-400 truncate max-w-[130px]" title={file.chapter}><span className="font-bold mr-1 text-[8.5px] uppercase tracking-wide opacity-75">{t("Ch:")}</span>{file.chapter}</p>)}
                                  {file.topic && (<p className="text-[10px] text-[#15803d] dark:text-brand-450 font-semibold truncate max-w-[130px]" title={file.topic}><span className="font-bold mr-1 text-[8.5px] uppercase tracking-wide opacity-75">{t("Topic:")}</span>{file.topic}</p>)}
                                </div>
                              </td>
                              <td className="py-3 px-4 text-center font-mono text-[11px] text-gray-600 dark:text-gray-400 whitespace-nowrap">{formatSizeMap(file.fileSize)}</td>
                              <td className="py-3 px-4 text-center">
                                {isDocApproved ? (<span className="inline-flex items-center gap-1 text-[9px] font-bold text-emerald-650 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-955/20 px-2 py-0.5 rounded-full border border-emerald-100 dark:border-emerald-900/30">{t("Approved")}</span>) : (<span className="inline-flex items-center gap-1 text-[9px] font-bold text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-955/20 px-2 py-0.5 rounded-full border border-amber-100 dark:border-amber-900/20">{t("Pending")}</span>)}
                              </td>
                              <td className="py-3 px-4 text-center font-mono font-bold text-gray-750 dark:text-gray-300">{file.downloadCount}</td>
                              <td className="py-3 px-4 text-right pr-6 whitespace-nowrap">
                                <div className="flex items-center justify-end gap-1.5">
                                  ...
                                  {file.uploadedBy === user.uid && (
                                    <>
                                      {onFileEdit && (
                                        <button onClick={() => handleOpenEdit(file)} className="p-1.5 bg-blue-50 dark:bg-blue-950/20 hover:bg-blue-100 dark:hover:bg-blue-950/40 text-blue-600 dark:text-blue-400 rounded border border-blue-100 dark:border-blue-900/30 cursor-pointer" title={t("Edit")}><Pencil className="w-3.5 h-3.5" /></button>
                                      )}
                                      <button onClick={() => onFileDelete(file.id)} className="p-1.5 bg-red-50 dark:bg-red-955/20 hover:bg-red-100 dark:hover:bg-red-955/40 text-red-650 dark:text-red-400 rounded border border-red-105 dark:border-red-900/30 cursor-pointer" title={t("Delete")}><Trash2 className="w-3.5 h-3.5" /></button>
                                    </>
                                  )}
                                </div>
                              </td>
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs transition-opacity animate-in fade-in duration-200" id="rejection-reasons-modal">
          <div className="relative bg-white dark:bg-slate-900 rounded-2xl w-full max-w-2xl px-6 py-6 border border-gray-150 dark:border-slate-800 shadow-2xl space-y-4 text-left animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between border-b border-gray-100 dark:border-slate-800 pb-4">
              <h3 className="font-extrabold text-base text-gray-805 dark:text-gray-105 flex items-center gap-2"><Bell className="w-5 h-5 text-red-505" /><span>{t("File Rejection Audit Records")}</span></h3>
              <button onClick={() => setShowRejectionModal(false)} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 focus:outline-none p-1 bg-gray-50 dark:bg-slate-850 rounded-lg cursor-pointer"><X className="w-4 h-4" /></button>
            </div>

            <div className="space-y-4 max-h-[450px] overflow-y-auto pr-2 custom-scrollbar">
              {loadingRejections ? (
                <div className="text-center py-12 text-xs font-semibold text-gray-400 dark:text-gray-500 flex flex-col items-center gap-2"><Loader2 className="w-5 h-5 animate-spin text-brand-500" /><span>{t("Scanning server archives...")}</span></div>
              ) : rejectionLogs.length === 0 ? (
                <div className="text-center py-12 text-xs text-gray-400 dark:text-gray-550 font-semibold space-y-2">
                  <div className="inline-flex p-3 bg-green-50 dark:bg-green-955/20 text-green-600 dark:text-green-400 rounded-full"><CheckCircle2 className="w-6 h-6" /></div>
                  <p>{t("Amazing! You do not have any rejected submissions.")}</p>
                </div>
              ) : (
                rejectionLogs.map((log) => (
                  <div key={log.id} className="p-4 bg-gray-50 dark:bg-slate-850 border border-gray-150 dark:border-slate-800 rounded-xl space-y-3">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-gray-100 dark:border-slate-800 pb-2">
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-1 text-[10px] font-bold text-gray-400 uppercase tracking-wide"><span>{t(log.fileSubject)}</span><span>•</span><span>{t(log.fileBranch)}</span></div>
                      </div>
                      <div className="flex items-center gap-1.5 text-[10px] text-gray-400 font-bold bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 px-2.5 py-1 rounded-md max-w-fit shadow-2xs"><Calendar className="w-3.5 h-3.5" /><span>{log.createdAt ? log.createdAt.toLocaleDateString() : "Just now"}</span></div>
                    </div>
                    <div className="bg-red-50/50 dark:bg-red-955/10 border-l-4 border-red-500 p-3 rounded-r-lg space-y-1">
                      <p className="text-[10px] uppercase font-bold text-red-600 dark:text-red-400 tracking-wider flex items-center gap-1.5"><AlertTriangle className="w-3.5 h-3.5 text-red-550" /><span>{t("Reason for Rejection")}</span></p>
                      <p className="text-xs text-red-700 dark:text-red-305 font-semibold whitespace-pre-wrap leading-relaxed">{log.rejectionReason}</p>
                    </div>
                    <div className="flex items-center justify-between text-[10px] text-gray-400 font-bold uppercase tracking-wider"><span>{t("Rejected by:")} <strong className="text-gray-600 dark:text-gray-300">{log.actorName}</strong> ({t(log.actorRole)})</span></div>
                  </div>
                ))
              )}
            </div>
            ...
            <div className="flex justify-end pt-2">
              <button onClick={() => setShowRejectionModal(false)} className="px-4 py-2 bg-gray-100 hover:bg-gray-200 dark:bg-slate-800 dark:hover:bg-slate-750 text-gray-700 dark:text-gray-200 text-xs font-bold rounded-lg cursor-pointer transition-all uppercase tracking-wider border border-transparent dark:border-slate-700">{t("Dismiss")}</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-md p-6 border border-gray-100 dark:border-slate-800">
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-sm font-bold text-gray-800 dark:text-white flex items-center gap-2"><Pencil className="w-4 h-4 text-blue-500" />{t("Edit File Details")}</h3>
              <button onClick={() => setEditingFile(null)} className="p-1.5 hover:bg-gray-100 dark:hover:bg-slate-800 rounded-lg cursor-pointer"><X className="w-4 h-4 text-gray-500" /></button>
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" value={editFileName} onChange={e => setEditFileName(e.target.value)} className="w-full px-3 py-2 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg text-sm text-gray-800 dark:text-gray-100 focus:outline-none focus:border-brand-500" />
              </div>
              <div>
                ...
                <textarea value={editDescription} onChange={e => setEditDescription(e.target.value)} rows={3} className="w-full px-3 py-2 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg text-sm text-gray-800 dark:text-gray-100 focus:outline-none focus:border-brand-500" />
              </div>
              <div>
                ...
                <select value={editSubject} onChange={e => setEditSubject(e.target.value)} className="w-full px-3 py-2 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg text-sm text-gray-800 dark:text-gray-100 focus:outline-none focus:border-brand-500">
                  {(teacherSubjects.length > 0 ? teacherSubjects : subjects).map(s => (<option key={s} value={s}>{t(s)}</option>))}
                </select>
              </div>
              <div>
                ...
                <select value={editClassLevel} onChange={e => setEditClassLevel(e.target.value)} className="w-full px-3 py-2 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg text-sm text-gray-800 dark:text-gray-100 focus:outline-none focus:border-brand-500">
                  <option value="">{t("-- Select Class --")}</option>
                  {getAssignedClassesForSubject(editSubject).map(c => (<option key={c} value={c}>{t(c)}</option>))}
                </select>
              </div>
            </div>

            <div className="flex gap-3 mt-6">
              <button onClick={() => setEditingFile(null)} className="flex-1 px-4 py-2.5 bg-gray-100 hover:bg-gray-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-gray-700 dark:text-gray-300 text-xs font-bold rounded-lg cursor-pointer transition-all">{t("Cancel")}</button>
              <button onClick={handleSaveEdit} disabled={editSaving || !editFileName.trim()} className="flex-1 px-4 py-2.5 bg-brand-500 hover:bg-brand-600 text-white text-xs font-bold rounded-lg cursor-pointer transition-all disabled:opacity-50 flex items-center justify-center gap-1.5">
