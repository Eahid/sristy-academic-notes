...
export default function DashboardAdmin({
  ...
  // States for editing a teacher's subjects & classes assignments
  ...
  const [editSelSubject, setEditSelSubject] = useState('');
  ...
  const handleSaveAssignments = async () => {
    ...
    if (editAssignments.length === 0) {
      alert(t("Teachers must specify at least one Subject and Class mapping."));
      return;
    }
    setSavingEdit(true);
    try {
      ...
      await updateDoc(doc(db, 'users', editingTeacher.uid), {
        classAssignments: updatedClassAssignments,
        subjects: updatedSubjects,
        classes: updatedClasses,
        subject: updatedSubject,
      });

      alert(t("Teacher assignments updated successfully."));
      setEditingTeacher(null);
      fetchBranchTeachers();
    } catch (err) {
      console.error(err);
      alert(t("Failed to update teacher assignments."));
    } finally {
      setSavingEdit(false);
    }
  };
  ...
  return (
    <div className="space-y-8" id="branch-admin-dashboard">
                                            ...
                                            {tea.role === 'teacher' && (
                                              <button
                                                ...
                                                onClick={(e) => {
                                                  ...
                                                  setEditAssignments(tea.classAssignments || []);
                                                  setEditSelSubject('');
                                                  setEditSelClass('');
                                                }}
      ...
      <AnimatePresence>
        {editingTeacher && (
          <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md overflow-hidden select-none" id="edit-teacher-assignments-modal">
            <motion.div
              initial={{ scale: 0.95, y: 15, opacity: 0 }}
              animate={{ scale: 1, y: 0, opacity: 1 }}
              exit={{ scale: 0.95, y: 15, opacity: 0 }}
              className="relative bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl w-full max-w-lg shadow-2xl flex flex-col text-gray-900 dark:text-gray-100 max-h-[90vh] overflow-hidden"
            >
              {/* Header */}
              <div className="bg-[#15803d] px-6 py-4 flex justify-between items-center text-white shrink-0">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="font-semibold text-base tracking-tight">{t("Edit Teacher Assignments")}</h2>
                </div>
                <button 
              ...
              <div className="p-6 overflow-y-auto space-y-5 text-left flex-1">
                <div>
                  <h3 className="font-bold text-sm text-gray-800 dark:text-gray-100">{editingTeacher.fullName}</h3>
                  <p className="text-xxs text-gray-400 dark:text-gray-500 font-mono">@{editingTeacher.username} • {editingTeacher.email}</p>
                </div>

                {/* Current assignments list */}
                <div className="space-y-2">
                  <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider">{t("Active Subjects & Class Levels")}</label>
                  {editAssignments.length === 0 ? (
                    <div className="p-4 rounded-xl border border-dashed border-gray-200 dark:border-slate-850 text-center text-xs text-gray-400 dark:text-gray-500">
                      {t("No active subject mappings. Add one below.")}
                    </div>
                  ) : (
                    <div className="space-y-1.5 max-h-[160px] overflow-y-auto scrollbar-none pr-1">
                      {editAssignments.map((asg, idx) => (
                        <div key={idx} className="flex items-center justify-between p-2.5 bg-gray-55/40 dark:bg-slate-950/20 border border-gray-100 dark:border-slate-850 rounded-xl hover:border-brand-100 transition-colors">
                          <div className="flex items-center gap-2 text-xs font-bold text-gray-800 dark:text-gray-200">
                            <span className="bg-indigo-50 dark:bg-indigo-950/20 text-indigo-600 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-900/30 px-2 py-0.5 rounded-md">
                              {t(asg.subject)}
                            </span>
                            <span className="text-gray-400 font-normal">➔</span>
                            <span className="text-[#15803d] dark:text-brand-400 bg-emerald-50 dark:bg-emerald-950/20 px-2 py-0.5 rounded-md">
                              {t(asg.classLevel)}
                            </span>
                          </div>
                          <button
                            ...
                            className="p-1 text-red-500 hover:text-red-700 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-955/25 rounded-md transition-colors cursor-pointer"
                ...
                <div className="bg-gray-50/50 dark:bg-slate-950/15 border border-gray-100 dark:border-slate-850 p-4 rounded-xl space-y-3.5">
                  <h4 className="text-xs font-bold text-gray-700 dark:text-gray-300 flex items-center gap-1.5">
                    ...
                    <span>{t("Assign New Subject & Class")}</span>
                  </h4>

                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-[9px] font-bold text-gray-400 uppercase tracking-wider mb-1">{t("Select Subject")}</label>
                      <select
                        value={editSelSubject}
                        onChange={(e) => setEditSelSubject(e.target.value)}
                    ...
                    <div>
                      <label className="block text-[9px] font-bold text-gray-400 uppercase tracking-wider mb-1">{t("Select Class")}</label>
                      <select
                        value={editSelClass}
                        onChange={(e) => setEditSelClass(e.target.value)}
                        className="w-full bg-white dark:bg-slate-850 border border-gray-200 dark:border-slate-700 rounded-lg p-2 text-xs font-semibold outline-none focus:border-[#15803d]"
                      >
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      if (!editSelSubject || !editSelClass) {
                        alert(t("Please select both a Subject and a Class level."));
                        return;
                      }
                      const exists = editAssignments.some(a => a.subject === editSelSubject && a.classLevel === editSelClass);
                      if (exists) {
                        alert(t("This subject & class mapping is already assigned to this teacher."));
                        return;
                      }
                      setEditAssignments([...editAssignments, { subject: editSelSubject, classLevel: editSelClass }]);
                      setEditSelSubject('');
                      setEditSelClass('');
                    }}
                    className="w-full py-2 bg-[#15803d] hover:bg-emerald-700 text-white font-bold text-xs rounded-lg transition-colors cursor-pointer flex items-center justify-center gap-1.5"
                  >
                    <PlusCircle className="w-3.5 h-3.5" />
                    <span>{t("Add to Teacher Assignments")}</span>
                  </button>
                </div>
              </div>

              {/* Footer */}
              <div className="p-4 bg-gray-50 dark:bg-slate-950/40 border-t border-gray-100 dark:border-slate-800 flex justify-end gap-2.5 shrink-0">
                <button
                  type="button"
                  onClick={() => setEditingTeacher(null)}
                  className="px-5 py-2.5 bg-gray-100 hover:bg-gray-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-gray-700 dark:text-gray-300 rounded-xl font-bold text-xs shadow-xs transition-colors cursor-pointer"
                  disabled={savingEdit}
                >
                  {t("Cancel")}
                </button>
                <button
                  ...
                  className="px-5 py-2.5 bg-[#15803d] hover:bg-emerald-700 text-white rounded-xl font-bold text-xs shadow-xs transition-colors cursor-pointer flex items-center gap-1.5"
                  disabled={savingEdit}
                >
                  {savingEdit ? (
                    <>
